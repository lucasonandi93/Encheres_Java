...
public class ArticleManager implements Manager<Article, Integer> {
	...
	public void validateData(Article data, BusinessException businessException) throws BusinessException {
		...
		if (data.getNameArticle() == null || data.getNameArticle().equalsIgnoreCase("")
				|| data.getNameArticle().length() > 30) {
		...
		if (data.getDescription() == null || data.getDescription().equalsIgnoreCase("")
				|| data.getDescription().length() > 300) {
		...
		if (data.getWithdrawal().getStreet() == null || data.getWithdrawal().getStreet().equalsIgnoreCase("")
				|| data.getWithdrawal().getStreet().length() > 30) {
		...
		if (data.getWithdrawal().getCp() == null || data.getWithdrawal().getCp().equalsIgnoreCase("")
				|| data.getWithdrawal().getCp().length() > 10) {
			businessException.addError(CodesResultatBLL.RULE_ARTICLE_WITHDRAWAL_ERROR);
		}

		// Vérification que la ville du lieu de retrait de l'article est valide
		if (data.getWithdrawal().getCity() == null || data.getWithdrawal().getCity().equalsIgnoreCase("")
				|| data.getWithdrawal().getCity().length() > 30) {
			businessException.addError(CodesResultatBLL.RULE_ARTICLE_WITHDRAWAL_ERROR);
		}
	}
